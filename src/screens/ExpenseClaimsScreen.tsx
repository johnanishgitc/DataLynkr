...
export default function ExpenseClaimsScreen() {
  ...
  const s3Attachment = useS3Attachment({ type: 'others', debug: true, debugLabel: '[ExpenseClaims][attachments]' });
  ...
  const fetchExpenseLedgers = useCallback(async () => {
    if (expenseLedgersLoading) return;
    setExpenseLedgersLoading(true);
    setExpenseLedgersError(null);
    try {
      const [tallyloc_id, company, guid] = await Promise.all([getTallylocId(), getCompany(), getGuid()]);
      if (!tallyloc_id || !company || !guid) {
        setExpenseLedgerNames([]);
        setExpenseLedgersError('Please select a company connection first.');
        return;
      }
      const res = await apiService.getExpenseLedgers({ tallyloc_id, company, guid });
      const rows = (res.data?.data ?? []).map((r) => String(r?.name ?? '').trim()).filter(Boolean);
      setExpenseLedgerNames(rows);
      if (rows.length === 0 && res.data?.success === false) {
        setExpenseLedgersError('No expense ledgers found.');
      }
    } catch (e: unknown) {
      if (isUnauthorizedError(e)) return;
      const msg = e && typeof e === 'object' && 'message' in e ? String((e as any).message) : 'Failed to load expense ledgers.';
      setExpenseLedgersError(msg);
      setExpenseLedgerNames([]);
    } finally {
  ...
  const onSidebarItemPress = useCallback(
    (item: AppSidebarMenuItem) => {
      closeSidebar();
      if (item.target === 'ExpenseClaims') return; // already here
      if (item.target === 'Payments' || item.target === 'Collections') {
  ...
  const handleSubmitForApproval = useCallback(async () => {
    ...
    if (!category.trim()) nextErrors.category = 'Please fill out this field';
  ...
  }, [voucherType, category, paymentMode, amount, notes, s3Attachment.attachments, resetForm]);
    ...
    <View style={s.root}>
      <View style={[s.headerWrap, { paddingTop: insets.top }]}>
        <View style={s.headerTopRow}>
          ...
          <Text style={s.headerTitle}>Expense Claims</Text>
        </View>
      </View>
      ...
      >
        ...
        <View style={s.fieldBlock}>
          <Text style={s.fieldLabel}>Expense Category</Text>
          <TouchableOpacity
            style={[s.selectBox, fieldErrors.category && s.selectBoxError]}
            onPress={() => {
              setCategoryOpen((v) => !v);
              if (expenseLedgerNames.length === 0 && !expenseLedgersLoading) fetchExpenseLedgers();
            }}
            activeOpacity={0.7}
          >
          ...
          {fieldErrors.category ? <Text style={s.fieldError}>{fieldErrors.category}</Text> : null}
          {false && categoryOpen && (
            <View style={s.inlineDropdown}>
              {expenseLedgersLoading ? (
                <View style={s.inlineDropdownLoading}>
        ...
        <View style={s.descBlock} ref={notesFieldRef}>
          ...
          <Text style={s.descHint}>This will be visible to your manager. ({notesRemaining} left)</Text>
        </View>
...
const s = StyleSheet.create({
  ...
  headerTitle: { fontFamily: 'Roboto', fontSize: 17, fontWeight: '600', color: colors.white },
  ...
  selectText: { flex: 1, fontFamily: 'Roboto', fontSize: 14, fontWeight: '400', color: colors.text_secondary, paddingVertical: 0 },
