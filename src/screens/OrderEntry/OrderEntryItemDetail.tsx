...
export default function OrderEntryItemDetail() {
  ...
  /** True after the shared units API request settles — until then qty/UOM must not be committed to cart. */
  const [unitsFetchDone, setUnitsFetchDone] = useState(false);
  ...
  const [enteredAddlQty, setEnteredAddlQty] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setUnitsFetchDone(true);
      }
    })();
  ...
  useEffect(() => {
    ...
    if (config) {
      const source = (editOrderItems != null && editOrderItems.length > 0) ? editOrderItems[0] : editOrderItem;
      if (source?.rateUnit) {
        rateUOMFromApi = getRateUOMFromUnitName(source.rateUnit, config, units);
      }
      if (!rateUOMFromApi && s) {
        if (selectedLedger) {
  ...
  }, [item?.stockItem?.MASTERID ?? item?.name ?? null, units, unitsFetchDone, editOrderItem, editOrderItems, selectedLedger]);
      ...
      setLineItems([
        {
          ...
          enteredQty: editOrderItem.enteredQty,
          rateUnit: editOrderItem.rateUnit,
  ...
  useEffect(() => {
    ...
    if (!selectedItemUnitConfig) {
      const interim = parseQuantityInput(quantityInput, null, units);
      const n = interim.qty;
      setItemQuantity(typeof n === 'number' && Number.isFinite(n) && n >= 0 ? n : 0);
      setCompoundBaseQty(null);
      setCompoundAddlQty(null);
      setBaseQtyOnly(null);
      setCustomAddlQty(null);
    ...
    setEnteredAddlQty(parsed.uom === 'additional' && parsed.qty != null ? parsed.qty : null);
  ...
  useEffect(() => {
    ...
    setValue(Number.isFinite(total) ? total.toFixed(2) : '0');
  }, [itemQuantity, rate, discount, rateUOM, selectedItemUnitConfig, units, compoundBaseQty, compoundAddlQty, baseQtyOnly, enteredAddlQty, customAddlQty, customConversion]);
  ...
  /** Until units are fetched and unit config exists, parsed qty / rate UOM can disagree with the field — block cart actions. */
  const qtyUomReady = isToBeAllocated || (unitsFetchDone && selectedItemUnitConfig != null);
  ...
  const handleAddToOrder = useCallback(() => {
    if (!isToBeAllocated && !qtyUomReady) {
      return;
    }
    ...
    const singleItemFromForm = (): AddedOrderItemWithStock => ({
      ...
      enteredQty: isToBeAllocated ? undefined : quantityInput.trim() || undefined,
      rate: String(r),
      unit: baseUnit,
      discount: d,
      total: formTotal,
    ...
    const toAddedItem = (line: OrderLineItem): AddedOrderItemWithStock => ({
      ...
      enteredQty: isToBeAllocated ? undefined : line.enteredQty,
  ...
  const handleAddItem = useCallback(() => {
    ...
    setLineItems((prev) => [
      ...prev,
      {
        ...
        enteredQty: quantityInput.trim() || undefined,
        rate: r,
        discount: d,
        total,
        stock: stockNum,
    ...
    s3Attachment.setAllAttachments([]);
  }, [name, isToBeAllocated, isQuickOrder, qtyUomReady, itemQuantity, quantityInput, rate, discount, value, stockNum, taxNum, nextId, dueDate, mfgDate, expiryDate, godown, batch, description, selectedItemUnitConfig, attachmentLinks, attachmentUris, route.params?.defaultGodown]);
  ...
  }, [selectedLineId, isToBeAllocated, isQuickOrder, qtyUomReady, itemQuantity, quantityInput, rate, discount, value, dueDate, mfgDate, expiryDate, godown, batch, description, selectedItemUnitConfig, attachmentLinks, attachmentUris]);
                        ...
                        onBlur={() => {
                          ...
                          if (validated && selectedItemUnitConfig) {
                            ...
                            if (!hasFreeText && parsed.isCustomConversion && parsed.qty != null && parsed.customAddlQty != null) {
                              ...
                              const fmtAddl = addlDec === 0 ? String(Math.round(parsed.customAddlQty)) : parsed.customAddlQty.toFixed(Number(addlDec));
              ...
              <View style={styles.buttonsRow}>
                ...
                <TouchableOpacity
                  style={[
                    styles.addItemBtn,
                    { flex: 1.5 },
                    (uploadingAttachments || (!isToBeAllocated && !qtyUomReady)) && { opacity: 0.5 },
                  ]}
                  onPress={selectedLineId != null ? handleUpdateItem : handleAddItem}
                  activeOpacity={0.8}
                  disabled={uploadingAttachments || (!isToBeAllocated && !qtyUomReady)}
                >
